...
function App() {
  ...
  function handleChangeComplete(color, event) {
    if(color.hex !== "#000000") client.publish(`iot/lights/${activeLight.id}`, color.hex);
  };
  ...
  return (
    <>
      ...
      <div className="lights-container">
        {lights.map(light => <Light key={light.id} light={light} setLights={setLights} setActiveLight={setActiveLight} />)}
        <NewLight lights={lights} setLights={setLights} />
      </div>
      <ChromePicker color={newColor} onChange={setNewColor} onChangeComplete={handleChangeComplete} disableAlpha={true} />
    </>
  );
}
