...
function NewLight({ lights, setLights }) {
  async function handleClick() {
    ...
    const homebridgeConfig = await axios.get(
      `${host}/api/config-editor/plugin/homebridge-mqttthing`,
      {
        headers: {
    ...
    setLights((oldLights) => [
      ...oldLights,
      { id: oldLights.length + 1, color: "#000000" },
    ]);
  }
  return (
    <button className="new-light" onClick={handleClick}>
      New Light
    </button>
  );
}
